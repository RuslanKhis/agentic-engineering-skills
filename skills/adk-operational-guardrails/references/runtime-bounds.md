...
Capture or render a final-answer event, then continue to invocation completion within that deadline unless deliberately cancelling. `is_final_response()` describes an event; it does not prove the iterator is exhausted or all state and usage have been recorded. On cancellation, close the iterator and retain uncertain usage for reconciliation.
...
Use a tool wrapper or a tested pre-execution callback/plugin for permission and repeat checks. Observe function calls/responses through ADK's actual APIs and correlate by call ID, including parallel calls. Admit only complete function calls; partial streaming events must cause zero tool executions. A stored function-call event is not proof of tool execution; the historical companion event guard was effective under its tested ADK ordering. Preserve that regression before changing an existing event guard or upgrading ADK.
...
Add deterministic event-consumption cases when adapting the serving wrapper: a final-answer event followed by a usage/state event must preserve both; a partial function-call event followed by its complete form must execute the tool only once; cancellation must run iterator cleanup and leave uncertain usage explicit. Use a scripted model or iterator and block provider network access. These are acceptance cases for the target integration, not claims that the bundled fixture already covers them.
