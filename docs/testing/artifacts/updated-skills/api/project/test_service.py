...
class GatewayTestCase(unittest.IsolatedAsyncioTestCase):
    ...
    def assert_no_leak(self, result, logs, marker=SECRET):
        # Check the whole record, including every structured extra field.
        text = json.dumps(result, default=str) + "\n".join(
            f"{record.getMessage()} {record.__dict__!r}" for record in logs.records
        )
...
class LookupTests(GatewayTestCase):
    ...
    async def test_unexpected_defect_is_logged_as_error_without_message(self):
        ...
        self.assertTrue(any(frame.endswith(":broken")
                            for frame in fields["error_frames"]))
        self.assert_no_leak(result, logs)
...
class RefundTests(GatewayTestCase):
    ...
    async def test_invalid_amounts_make_no_request(self):
        ...
        amounts = [10, 10.0, Decimal("10.00"), True, None, "10", "10.0", "10.001",
                   "0.00", "00.00", "+0.00", "-0.00", "-1.00", "++1.00", "+-1.00",
                   "1.", ".50", "1e3", "NaN", "Infinity", "+Infinity", "sNaN",
                   "0.001", "0.009",
    ...
    async def test_boundary_amounts_are_accepted(self):
        for amount in ("0.01", "999999999999999.99", "01.00", "+1.00",
                       "+01.00", "000000000000001.00"):
            with self.subTest(amount=amount):
                ...
                self.assert_schema(result, True, "applied")
                [request] = provider.requests
                # The caller's spelling is sent unchanged, not normalised.
                self.assertEqual(json.loads(request.content)["amount"], amount)

    async def test_normalised_amount_echo_is_unknown(self):
        for submitted, echoed in (("01.00", "1.00"), ("+1.00", "1.00"),
                                  ("1.00", "+1.00")):
            with self.subTest(submitted=submitted), self.assertLogs("service"):
                gateway, provider, _ = self.gateway(
                    respond(200, applied_body(amount=echoed)))
                result = await gateway.refund(REFERENCE, submitted)
                self.assert_schema(result, False, "unknown")
                self.assertIn("invalid_response:200:amount", result["reason"])
                self.assertEqual(len(provider.requests), 1)
...
class SourceLiteralLeakTests(unittest.IsolatedAsyncioTestCase):
    """A literal in the failing handler's source must not reach log records."""

    CANARY = "PRIVATE_EXCEPTION_PAYLOAD_CANARY"

    async def run_with_handler(self, call):
        from defect_handler import raising_handler
        async with httpx.AsyncClient(
            transport=httpx.MockTransport(raising_handler),
            base_url="https://provider.test",
        ) as client:
            with self.assertLogs("service", logging.ERROR) as logs:
                result = await call(Gateway(client))
        return result, logs

    def assert_safe(self, result, logs):
        rendered = json.dumps(result) + "".join(
            f"{r.getMessage()}{r.__dict__!r}" for r in logs.records)
        self.assertNotIn(self.CANARY, rendered)
        [record] = logs.records
        fields = record.provider_call
        self.assertEqual(fields["error_class"], "RuntimeError")
        self.assertEqual(fields["reason_code"], "unexpected_error")
        self.assertIsNone(record.exc_info)
        self.assertIn("defect_handler.py:7:raising_handler", fields["error_frames"])
        for frame in fields["error_frames"]:
            file_name, line, function = frame.split(":")
            self.assertTrue(file_name.endswith(".py"))
            self.assertTrue(line.isdigit())
            self.assertTrue(function.isidentifier() or function.startswith("<"))

    async def test_lookup_defect_logs_locations_only(self):
        result, logs = await self.run_with_handler(lambda g: g.lookup(REFERENCE))
        self.assertEqual(result["reason"], "unexpected_error")
        self.assert_safe(result, logs)

    async def test_refund_defect_logs_locations_only(self):
        result, logs = await self.run_with_handler(
            lambda g: g.refund(REFERENCE, "10.00"))
