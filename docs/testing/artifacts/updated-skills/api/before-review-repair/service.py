...
# Fixed USD fixture: canonical positive decimal string with exactly two
# fractional digits; the integer-digit bound limits input size, not business.
...
def _log(level, operation, outcome, started, failure=None, error=None):
    ...
    if error is not None:
        fields.update(
            ...
            # Frames only: the exception message may contain sensitive data.
            error_frames="".join(traceback.format_tb(error.__traceback__)),
        )
    logger.log(
        level,
