...
2. **Confirmation Boundary**:
   - For a new operation, `confirmed=False` returns cancelled with zero provider calls (`{"status": "cancelled", "operation_id": ..., "amount_minor": ..., "refund_id": None}`). A previously dispatched operation with the same amount returns its stored outcome.
